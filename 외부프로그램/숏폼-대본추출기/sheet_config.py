...
VIDEO_LIST_HEADERS = [
    ...
    "대본유무",           # 52: 'ㅇ' 또는 'x' (전역함수 열 - 자동 계산)
    "대본 텍스트수",      # 53: 글자 수 (전역함수 열 - 자동 계산)
    "구글닥스여부",       # 54: Google Docs 업로드 여부 (데이터 입력 열)
    "닥스파일ID",         # 55: Google Docs 파일 ID (데이터 입력 열)
    "닥스파일경로",       # 56: Google Docs URL (데이터 입력 열)
    "대본txt여부",        # 57: TXT 파일 업로드 여부 (데이터 입력 열)
    "대본txtID",          # 58: TXT 파일 ID (데이터 입력 열)
    "대본txt경로",        # 59: TXT 파일 경로 (데이터 입력 열)
    ...
    # ===== 썸네일 관련 열 (62-64) =====
    "썸네일여부",         # 62: 썸네일 다운로드 여부 (데이터 입력 열)
    "이미지주소",         # 63: YouTube 썸네일 URL (데이터 입력 열)
    "경로",              # 64: Google Drive 썸네일 경로 (데이터 입력 열)
...
# 주의: 이 열들은 데이터 삽입 시 건너뛰어야 합니다 (9행의 전역함수가 자동 계산)
