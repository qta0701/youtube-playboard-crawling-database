...
class GoogleSheetsManager:
    ...
    def __init__(self, credentials_path: str):
        ...
        self.available_spreadsheets = {
            '채널수집_2.국내롱폼': 'https://docs.google.com/spreadsheets/d/15VuwgxcfjWUGzlQIstSNcK5SnTQZtaEi3XRfyffPxEE/edit?gid=1434237297#gid=1434237297',
            '채널수집_3.유튜브 노하우 재생목록': 'https://docs.google.com/spreadsheets/d/1hhgcMFS5v4F5ViBSBtj0ZfEFYdAFxFQz2HO1MUlZy28/edit?gid=1434237297#gid=1434237297'
        }
        ...
        # 캐싱 추가 - API 호출 최소화
        self._sheet_cache = {}  # {(sheet_url, sheet_name): worksheet}
        self._header_cache = {}  # {(sheet_url, sheet_name): headers}
        self._metadata_cache = {}  # {(sheet_url, sheet_name, row): metadata}
        self._playlist_name_cache = {}  # {(sheet_url, sheet_name, row): playlist_name}
        self._folder_cache = {}  # {(folder_name, parent_id): folder_id} - Drive 폴더 캐싱

        # API Rate Limit 관리
        self._api_call_count = 0
        self._api_call_reset_time = time.time() + 60  # 1분마다 리셋
        
    def _wait_for_rate_limit(self):
        """API Rate Limit 관리 - 분당 60회 제한"""
        current_time = time.time()

        # 1분 경과 시 카운터 리셋
        if current_time >= self._api_call_reset_time:
            self._api_call_count = 0
            self._api_call_reset_time = current_time + 60

        # 55회 이상 호출 시 대기 (안전 마진)
        if self._api_call_count >= 55:
            wait_time = self._api_call_reset_time - current_time
            if wait_time > 0:
                logger.warning(f"⏳ API Rate Limit 근접 - {wait_time:.1f}초 대기 중...")
                time.sleep(wait_time + 1)
                self._api_call_count = 0
                self._api_call_reset_time = time.time() + 60

        self._api_call_count += 1

    def _api_call_with_retry(self, func, *args, max_retries=5, **kwargs):
        """API 호출 시 429 에러 처리 및 지수 백오프"""
        for attempt in range(max_retries):
            try:
                ...
                if '429' in error_str or 'RATE_LIMIT_EXCEEDED' in error_str or 'Quota exceeded' in error_str:
                    ...
                    logger.warning(f"⚠️ API Rate Limit 초과 - {wait_time}초 대기 후 재시도 ({attempt + 1}/{max_retries})")
                ...
                # invalid_grant 에러 처리 (OAuth 토큰 만료)
                elif 'invalid_grant' in error_str:
                    logger.warning("⚠️ OAuth 토큰 만료 - 재인증 시도")
                    try:
                        # 토큰 파일 삭제하고 재인증
                        ...
                        logger.error(f"❌ OAuth 재인증 실패: {auth_error}")
                        raise
        ...
        # 모든 재시도 실패
        raise Exception(f"API 호출 실패 - {max_retries}번 재시도 후에도 실패")
            ...
            if not self.oauth_credentials or not self.oauth_credentials.valid:
                if self.oauth_credentials and self.oauth_credentials.expired and self.oauth_credentials.refresh_token:
                    ...
                    try:
                        ...
                        logger.warning(f"토큰 갱신 실패: {refresh_error}, 새 인증 시도...")
                        # 토큰 파일 삭제
                        if os.path.exists(self.token_path):
                            os.remove(self.token_path)
                        self.oauth_credentials = None
        ...
        try:
            ...
            if mode == 'A':
                ...
                if script_exists_col:
                    # 대본유무 열이 있으면 빈 행만 필터링
                    script_exists_values = sheet.col_values(script_exists_col)

                    filtered_ids = []
                    missing_rows = []
                    for i in range(9, len(video_ids)):  # 10행부터
                    ...
                    row_mapping = missing_rows
                    logger.info(f"모드 A: 10행부터 대본유무가 빈 {len(filtered_ids)}개 영상만 필터링")
                    if missing_rows and len(missing_rows) > 0:
                        logger.debug(f"처리할 행들: {missing_rows[:10]}{'...' if len(missing_rows) > 10 else ''}")
                else:
                    # 대본유무 열이 없으면 기존 방식대로
                    start_row = 10
                    filtered_ids = [vid.strip() for vid in video_ids[9:] if vid.strip()]
                    logger.debug(f"모드 A: 10행부터 {len(filtered_ids)}개 영상 ID 필터링 (대본유무 열 없음)")
            elif mode == 'B':
        ...
        try:
            ...
            for i, h in enumerate(headers, 1):
                if '제목' in str(h):
                    title_col = i
                    break
            logger.info(f"📋 헤더 캐싱 완료: 제목 열={title_col}")

            # 모든 행 데이터를 한 번에 가져와서 캐싱 (API Rate Limit 방지)
            all_values = sheet.get_all_values()  # 전체 데이터 한 번에 가져오기
            logger.info(f"📊 시트 데이터 캐싱 완료: {len(all_values)}행")

            # 제목 가져오기 헬퍼 함수 (캐시된 데이터 사용, API 호출 없음)
            def get_title_cached(current_row, video_id):
                if not title_col:
                    logger.warning(f"⚠️ 제목 열을 찾을 수 없습니다. video_id 사용: {video_id}")
                    return video_id
                try:
                    # all_values는 0-based 인덱스, current_row는 1-based
                    if current_row <= len(all_values):
                        row_data = all_values[current_row - 1]
                        if title_col <= len(row_data):
                            title = row_data[title_col - 1]
                            if title and title.strip():
                                logger.debug(f"📝 {current_row}행에서 제목 추출 (캐시): '{title.strip()}'")
                                return title.strip()
                    logger.warning(f"⚠️ {current_row}행의 제목이 비어있습니다. video_id 사용: {video_id}")
                    return video_id
                except Exception as e:
                    logger.error(f"❌ {current_row}행 제목 추출 실패: {e}, video_id 사용: {video_id}")
                    return video_id
            ...
            for i, video_data in enumerate(video_data_list):
                ...
                if video_data.transcript:
                    ...
                    MIN_TEXT_COUNT_FOR_DOCS = 9000  # 대본 텍스트 수 9000자 이상
                    MIN_DURATION_FOR_DOCS = 720  # 12분 = 720초

                    # 먼저 메타데이터를 가져와서 영상 길이 확인
                    should_save_to_drive = False
                    save_reason = ""

                    if len(transcript_text) > MAX_CELL_LENGTH:
                        should_save_to_drive = True
                        save_reason = f"대본 길이 초과 ({len(transcript_text)}자)"
                    elif len(transcript_text) > MIN_TEXT_COUNT_FOR_DOCS:
                        should_save_to_drive = True
                        save_reason = f"대본 텍스트 수 9000자 이상 ({len(transcript_text)}자)"

                    # 영상 길이 확인을 위해 메타데이터 미리 추출
                    if not should_save_to_drive:
                        try:
                            # 해당 행 번호 계산
                            if row_mapping and i < len(row_mapping):
                            ...
                            # 영상 길이가 12분 이상이면 드라이브에 저장
                            if metadata.get('duration_seconds', 0) >= MIN_DURATION_FOR_DOCS:
                                should_save_to_drive = True
                                save_reason = f"영상 길이 12분 이상 ({metadata.get('duration', 'N/A')})"
                                logger.info(f"📄 {video_data.video_id}: {save_reason} - 드라이브에 자동 저장")
                        except Exception as meta_error:
                            logger.warning(f"⚠️ {video_data.video_id}: 메타데이터 추출 실패 - {meta_error}")
                            metadata = {}

                    if should_save_to_drive:
                        # 긴 대본 또는 12분 이상 영상은 드라이브에 자동 저장
                        logger.info(f"📄 {video_data.video_id}: {save_reason} - 드라이브에 자동 저장")

                        try:
                            # 해당 행 번호 계산 (매 영상마다 새로 계산)
                            if row_mapping and i < len(row_mapping):
                                current_row = row_mapping[i]
                            else:
                                current_row = start_row + i

                            # 시트에서 제목 헤더 읽어오기 (캐시된 열 사용)
                            title = get_title_cached(current_row, video_data.video_id)

                            # 재생목록 이름 추출
                            playlist_name = self.get_playlist_name_from_sheet(sheet_url, sheet_name, current_row)

                            # 시트에서 메타데이터 추출 (매번 새로 추출하여 이전 영상 데이터와 혼동 방지)
                            try:
                                metadata = self.get_video_metadata_from_sheet(sheet_url, sheet_name, current_row, video_data.video_id)
                            except Exception as meta_error:
                                logger.warning(f"⚠️ {video_data.video_id}: 메타데이터 추출 실패 - {meta_error}")
                            ...
                            # 대본내용 셀에는 대체 메시지 입력
                            drive_message = f"드라이브 닥스 저장처리 ({save_reason})"
                            updates.append({
                                'range': cell_address,
                                'values': [[drive_message]]
                            })
                            ...
                            success_count += 1
                            logger.info(f"✅ {video_data.video_id}: {save_reason} - 드라이브 저장 완료")
                    ...
                    # 자막이 없는 경우들을 'x'로 표시
                    error_msg = str(video_data.error).lower()
                    no_transcript_keywords = [
                    ...
                    if any(keyword in error_msg for keyword in no_transcript_keywords):
                        updates.append({
                            'range': cell_address,
                            'values': [['x']]
                        })
                        error_count += 1
                        logger.debug(f"📝 {video_data.video_id}: {cell_address}에 'x' 업데이트 준비 (자막 없음: {video_data.error})")
                    else:
                        # 기타 에러는 에러 메시지 표시
                        updates.append({
                            'range': cell_address,
            ...
            if updates:
                ...
                # 전역함수 보호를 위해 헤더 정보 가져오기 (캐싱 적용)
                header_cache_key = (sheet_url, sheet_name)
                if header_cache_key in self._header_cache:
                    logger.debug("📦 캐시에서 헤더 정보 반환")
                    headers = self._header_cache[header_cache_key]
                else:
                    headers = self._api_call_with_retry(lambda: sheet.row_values(1))
                    self._header_cache[header_cache_key] = headers
                ...
                # 전역함수 헤더 열 비우기 (10행부터 마지막 데이터 행까지)
                self.clear_global_function_headers(sheet, start_row=10)
            else:
    ...
    def get_docs_extraction_data(self, sheet_url: str, sheet_name: str, max_count: int = 10, extraction_mode: str = "both") -> List[Dict]:
        ...
        try:
            ...
            field1_col = self.find_column_by_keyword(sheet, '분야1')  # 재생목록 이름 대체용
            ...
            if extraction_mode != "shorts_thumbnail":
                # Docs 관련 열 - 구글닥스 여부는 더 이상 업데이트하지 않음
                docs_status_col = self.find_column_by_keyword(sheet, '구글 닥스 여부') or self.find_column_by_keyword(sheet, '구글닥스 여부')
                # 대본파일 ID/경로로 변경
                docs_id_col = self.find_column_by_keyword(sheet, '대본파일 ID') or self.find_column_by_keyword(sheet, '닥스파일 ID')
                docs_path_col = self.find_column_by_keyword(sheet, '대본파일 경로') or self.find_column_by_keyword(sheet, '닥스파일 경로')
            ...
            # 재생목록 이름 또는 분야1 중 하나는 있어야 함
            if not playlist_col and not field1_col:
                logger.warning(f"⚠️ '재생목록 이름'과 '분야1' 열을 모두 찾을 수 없습니다. 폴더 구조 생성 불가")

            if not all([transcript_col, script_exists_col, title_col]):
                missing_cols = []
            ...
            logger.info(f"📊 기본 열 위치 - 대본내용: {transcript_col}, 대본유무: {script_exists_col}, 재생목록이름: {playlist_col}, 분야1: {field1_col}, 제목: {title_col}")
            if docs_status_col:
            ...
            for row_idx in range(9, len(all_values)):  # 10행부터 (인덱스 9)
                ...
                # 재생목록 이름 추출 (없으면 분야1 사용)
                playlist_name = ""
                if playlist_col and playlist_col - 1 < len(row):
                    playlist_name = row[playlist_col - 1]
                elif field1_col and field1_col - 1 < len(row):
                    playlist_name = row[field1_col - 1]
                    logger.debug(f"🔍 {row_idx + 1}행: 재생목록 이름 없음, 분야1 사용: '{playlist_name}'")
                ...
                if extraction_mode == "docs_only":
                    # Docs만 추출 모드: Docs가 비어있으면 처리
    ...
    def find_or_create_folder(self, folder_name: str, parent_folder_id: str = None) -> str:
        """구글 드라이브에서 폴더 찾기 또는 생성 (캐싱 적용)"""
        try:
            # 캐시 키 생성
            cache_key = (folder_name, parent_folder_id or "root")

            # 캐시에서 확인
            if cache_key in self._folder_cache:
                folder_id = self._folder_cache[cache_key]
                logger.debug(f"📦 캐시에서 폴더 ID 반환: '{folder_name}' (ID: {folder_id})")
                return folder_id

            logger.debug(f"📁 폴더 '{folder_name}' 찾기/생성 시도 (API 호출)")
    ...
    def create_drive_documents_for_long_transcript(self, video_id: str, title: str, transcript_text: str, playlist_name: str = None, metadata: dict = None, sheet_name: str = None) -> tuple:
        ...
        try:
            ...
            if sheet_name and '영상 리스트' in sheet_name and metadata and metadata.get('channel_name'):
                ...
                # 스프레드시트 이름 기반 폴더 구조 사용
                # metadata에 spreadsheet_name이 있으면 사용
                if metadata and metadata.get('spreadsheet_name'):
                    spreadsheet_name = metadata['spreadsheet_name']
                    logger.debug(f"📁 스프레드시트 폴더 '{spreadsheet_name}' 찾기/생성")
                    spreadsheet_folder_id = self.find_or_create_folder(spreadsheet_name, base_folder_id)

                    # 분야1(재생목록) 폴더 찾기/생성
                    logger.debug(f"📁 분야1 폴더 '{playlist_name}' 찾기/생성")
                    playlist_folder_id = self.find_or_create_folder(playlist_name, spreadsheet_folder_id)
                else:
                    # 기존 로직 (재생목록명으로 폴더 생성)
            ...
            if not existing_docs:
                logger.debug(f"📝 'ㅇ제목' 형식으로 기존 Google Docs 확인: ㅇ{safe_title}")
                existing_docs = self.find_existing_file(f"ㅇ{safe_title}", playlist_folder_id, 'application/vnd.google-apps.document')

            if existing_docs:
                docs_id = existing_docs['id']
                docs_url = f"https://docs.google.com/document/d/{docs_id}/edit"
                logger.info(f"📁 기존 Google Docs 재사용: '{existing_docs['name']}' (ID: {docs_id}) - 새 업로드 없이 시트 업데이트만 진행")
            else:
            ...
            # 중복 파일 확인 (TXT) - '제목.txt'와 'ㅇ제목.txt' 모두 검색
            ...
            # 'ㅇ제목.txt' 파일도 검색
            if not existing_txt:
                txt_filename_o = f"ㅇ{safe_title}.txt"
                logger.debug(f"📄 'ㅇ제목' 형식으로 기존 TXT 파일 확인: {txt_filename_o}")
                existing_txt = self.find_existing_file(txt_filename_o, txt_folder_id, 'text/plain')

            if existing_txt:
                txt_id = existing_txt['id']
                txt_url = f"https://drive.google.com/file/d/{txt_id}/view"
                logger.info(f"📁 기존 TXT 파일 재사용: '{existing_txt['name']}' (ID: {txt_id}) - 새 업로드 없이 시트 업데이트만 진행")
            else:
                # TXT 파일 생성
    ...
    def get_video_metadata_from_sheet(self, sheet_url: str, sheet_name: str, row: int, video_id: str) -> dict:
        """시트에서 비디오 메타데이터 추출 (API 호출 최적화: 행 전체를 한 번에 읽기)"""
        # 캐시 확인
        cache_key = (sheet_url, sheet_name, row)
        if cache_key in self._metadata_cache:
            logger.debug(f"📦 캐시에서 메타데이터 반환 ({video_id})")
        ...
        try:
            def _get_metadata():
                ...
                # ✅ API 호출 최적화: get_header_columns 사용 (이미 캐싱되어 있음)
                column_mapping = self.get_header_columns(sheet, use_cache=True, header_row_num=1)

                channel_col = column_mapping.get('채널명')
                views_col = column_mapping.get('조회수')
                upload_date_col = column_mapping.get('영상 업로드날짜') or column_mapping.get('영상 업로드 날짜')
                duration_col = column_mapping.get('영상길이')

                metadata = {}

                # ✅ API 호출 최적화: 행 전체를 한 번에 읽기 (개별 셀 읽기 대신)
                try:
                    row_values = sheet.row_values(row)
                    logger.debug(f"📄 행 {row} 전체 데이터 읽기 완료 ({len(row_values)}개 셀)")

                    # 각 컬럼에서 데이터 추출 (이미 읽은 row_values에서 추출)
                    if channel_col and channel_col <= len(row_values):
                        channel_value = row_values[channel_col - 1]  # 1-based to 0-based
                        if channel_value:
                            metadata['channel_name'] = str(channel_value).strip()
                            logger.debug(f"채널명 추출: {metadata['channel_name']}")

                    if views_col and views_col <= len(row_values):
                        views_value = row_values[views_col - 1]
                        if views_value:
                            metadata['view_count'] = str(views_value).strip()
                            logger.debug(f"조회수 추출: {metadata['view_count']}")

                    if upload_date_col and upload_date_col <= len(row_values):
                        upload_date_value = row_values[upload_date_col - 1]
                        if upload_date_value:
                            metadata['upload_date'] = str(upload_date_value).strip()
                            logger.debug(f"업로드 날짜 추출: {metadata['upload_date']}")

                    if duration_col and duration_col <= len(row_values):
                        duration_value = row_values[duration_col - 1]
                        if duration_value:
                            duration_str = str(duration_value).strip()
                            metadata['duration'] = duration_str
                            # 초 단위로도 저장
                            duration_seconds = self.parse_duration_to_seconds(duration_str)
                            metadata['duration_seconds'] = duration_seconds
                            logger.debug(f"영상길이 추출: {metadata['duration']} ({duration_seconds}초)")

                except Exception as e:
                    logger.warning(f"⚠️ 행 {row} 메타데이터 추출 중 오류: {e}")

                logger.info(f"🔍 {video_id}: 메타데이터 추출 완료 - 채널명: {metadata.get('channel_name', 'N/A')}, 조회수: {metadata.get('view_count', 'N/A')}, 업로드일: {metadata.get('upload_date', 'N/A')}, 영상길이: {metadata.get('duration', 'N/A')}")
                return metadata

            # API 호출 with retry
            result = self._api_call_with_retry(_get_metadata)
    ...
    def update_all_transcript_headers(self, sheet, video_data_list: List, row_mapping: Optional[List[int]], start_row: int):
        ...
        try:
            # 헤더를 한 번만 가져와서 로컬에서 검색 (API Rate Limit 방지)
            headers = sheet.row_values(1)

            # 디버깅: 20-25열 헤더 출력
            logger.info(f"📋 헤더 20-25열: {headers[19:25] if len(headers) >= 25 else headers[19:]}")

            def find_column_in_headers(keywords):
                """헤더 리스트에서 키워드를 포함하는 열 찾기 (로컬 검색)"""
                if not isinstance(keywords, list):
                    keywords = [keywords]

                for keyword in keywords:
                    for i, header in enumerate(headers, 1):
                        if keyword.lower() in str(header).lower().replace(' ', ''):
                            logger.debug(f"'{keyword}' 헤더 발견: {i}열 ('{header}')")
                            return i

                # 찾지 못한 경우 로그 출력
                logger.warning(f"⚠️ 헤더를 찾지 못함: {keywords}")
                return None

            # 필요한 열 찾기 (로컬 검색으로 API 호출 최소화)
            script_exists_col = find_column_in_headers(['대본유무'])
            docs_status_col = find_column_in_headers(['구글닥스여부', '구글 닥스 여부'])
            docs_file_id_col = find_column_in_headers(['대본파일id', '대본파일 id', '닥스파일id', '닥스파일 id'])
            docs_file_path_col = find_column_in_headers(['대본파일경로', '대본파일 경로', '닥스파일경로', '닥스파일 경로', '대본파일', '닥스파일', '파일경로'])
            txt_status_col = find_column_in_headers(['대본txt여부'])
            txt_file_id_col = find_column_in_headers(['대본txtid'])
            txt_file_path_col = find_column_in_headers(['대본txt경로'])
            thumbnail_status_col = find_column_in_headers(['썸네일여부', '썸네일 여부'])
            thumbnail_image_col = find_column_in_headers(['썸네일이미지주소', '썸네일 이미지주소', '이미지주소'])
            thumbnail_path_col = find_column_in_headers(['썸네일경로', '썸네일 경로', '경로'])

            logger.info(f"📊 헤더 열 위치 - 대본파일ID: {docs_file_id_col}, 대본파일경로: {docs_file_path_col}, 썸네일여부: {thumbnail_status_col}")
            ...
            for i, video_data in enumerate(video_data_list):
                ...
                if docs_info:  # 긴 대본인 경우
                    # 구글닥스 여부는 전역함수이므로 업데이트하지 않음
                    # (대본유무와 마찬가지로 전역함수가 자동으로 처리)

                    # 대본파일 ID/경로 업데이트
                    if docs_file_id_col and docs_info.get('id'):
                    ...
                    if docs_file_path_col and docs_info.get('url'):
                    ...
                    # TXT 관련 헤더 업데이트 (TXT는 표시하지 않음, 내부 처리용만)
                    # txt_info는 있어도 시트에 표시하지 않음

                # 썸네일 헤더 업데이트 (성공한 경우만)
                if thumbnail_info and thumbnail_info.get('success'):
                    # 썸네일 여부는 전역함수이므로 업데이트하지 않음
                    # (썸네일 이미지주소/썸네일 경로 기반으로 9행 전역함수가 자동 계산)
    ...
    def clear_global_function_headers(self, sheet, start_row: int = 10):
        """전역함수가 있는 헤더 열들을 10행부터 마지막 데이터 행까지 비우기"""
        try:
            logger.info(f"🧹 전역함수 헤더 열 비우기 시작 (시작행: {start_row})")
            ...
            # sheet_config.py에서 전역함수 헤더 목록 가져오기
            formula_columns = get_formula_columns(SheetType.VIDEO_LIST)
            logger.debug(f"📋 전역함수 헤더 목록 (sheet_config): {formula_columns}")

            # 전역함수 헤더 열 찾기
            global_function_cols = []
            for i, header in enumerate(headers, 1):
                if is_formula_column(header, SheetType.VIDEO_LIST):
                    global_function_cols.append(i)
                    logger.info(f"📋 전역함수 헤더 발견: {i}열 ('{header}')")

            if not global_function_cols:
                logger.info("⚠️ 비울 전역함수 헤더 열이 없습니다")
                return
            ...
            # 빈 행은 제외 (역순으로 확인하여 실제 마지막 데이터 행 찾기)
            for row_idx in range(len(all_values) - 1, start_row - 1, -1):
                if any(cell.strip() for cell in all_values[row_idx]):
                    last_data_row = row_idx + 1
                    break

            logger.info(f"📊 마지막 데이터 행: {last_data_row}행")

            if last_data_row < start_row:
                logger.info(f"⚠️ 시작행({start_row})보다 마지막 데이터 행({last_data_row})이 작습니다. 비우기 작업 건너뜀")
                return

            # 각 전역함수 열에 대해 비우기 작업 수행
            clear_updates = []
            for col in global_function_cols:
                col_letter = column_number_to_letter(col)
                # 범위 형식: "A10:A100" (예시)
                range_str = f"{col_letter}{start_row}:{col_letter}{last_data_row}"

                # 빈 값으로 채우기 위한 데이터 생성
                empty_values = [['']] * (last_data_row - start_row + 1)
                ...
                logger.debug(f"🧹 {col}열({col_letter}) {start_row}행~{last_data_row}행 비우기 준비")

            # 벌크 업데이트로 한 번에 비우기
            if clear_updates:
                logger.info(f"🧹 전역함수 헤더 열 비우기 실행: {len(clear_updates)}개 열, {start_row}행~{last_data_row}행")
                sheet.batch_update(clear_updates)
                logger.info(f"✅ 전역함수 헤더 열 비우기 완료")

        except Exception as e:
            logger.error(f"❌ 전역함수 헤더 열 비우기 실패: {e}")
            # 비우기 실패는 치명적이지 않으므로 예외를 다시 던지지 않음
    ...
    def update_additional_headers_for_long_transcripts(self, sheet, video_data_list: List, row_mapping: Optional[List[int]], start_row: int):
        ...
        try:
            ...
            # 필요한 헤더 열 찾기 (로컬 검색으로 API 호출 최소화)
            script_exists_col = find_column_in_headers(['대본유무'])  # 전역함수 헤더 - 업데이트 금지
            docs_status_col = find_column_in_headers(['구글닥스여부', '구글 닥스 여부'])  # 업데이트 금지
            # 대본파일 ID/경로로 변경
            docs_id_col = find_column_in_headers(['대본파일id', '대본파일 id', '닥스파일id', '닥스파일 id'])
            docs_path_col = find_column_in_headers(['대본파일경로', '대본파일 경로', '닥스파일경로', '닥스파일 경로'])
            # TXT 관련 헤더는 제거됨 - 무시
            txt_status_col = None
            txt_id_col = None
            txt_path_col = None
            ...
            # 썸네일 관련 열
            thumbnail_status_col = find_column_in_headers(['썸네일여부', '썸네일 여부'])
            thumbnail_image_col = find_column_in_headers(['썸네일이미지주소', '썸네일 이미지주소'])
            thumbnail_path_col = find_column_in_headers(['썸네일경로', '썸네일 경로'])
            ...
            for i, video_data in enumerate(video_data_list):
                ...
                # 대본유무 열은 전역함수이므로 업데이트 금지
                # 구글닥스 여부도 업데이트 금지

                # 대본파일 ID/경로 업데이트 (기존 닥스파일 ID/경로)
    ...
    def handle_long_transcript_for_browser_automation(self, video_id: str, title: str, transcript_text: str,
        ...
        try:
            ...
            # 영상 길이가 12분 이상이면 드라이브에 저장
            if not should_save_to_drive and metadata.get('duration_seconds', 0) >= MIN_DURATION_FOR_DOCS:
                should_save_to_drive = True
                save_reason = f"영상 길이 12분 이상 ({metadata.get('duration', 'N/A')})"

            if should_save_to_drive:
                logger.info(f"📄 {video_id}: {save_reason} - 드라이브에 자동 저장")
                ...
                logger.info(f"✅ {video_id}: {save_reason} - 드라이브 저장 및 헤더 업데이트 완료")
    ...
    def get_title_from_sheet(self, sheet_url: str, sheet_name: str, row: int, video_id: str) -> str:
        """시트의 특정 행에서 제목 추출"""
        try:
            def _get_title():
                workbook = self.client.open_by_url(sheet_url)
                ...
                if not title_col:
                    logger.warning(f"⚠️ '제목' 열을 찾을 수 없습니다. video_id 사용")
                    return video_id

                # 해당 행의 제목 가져오기
                title = sheet.cell(row, title_col).value
                if not title or not title.strip():
                    logger.warning(f"⚠️ {row}행의 제목이 비어있습니다. video_id 사용")
                    return video_id

                logger.debug(f"📝 {row}행에서 제목 추출: '{title.strip()}'")
                return title.strip()

            # API 호출 with retry
            result = self._api_call_with_retry(_get_title)
            return result

        except Exception as e:
            logger.error(f"❌ {row}행 제목 추출 실패: {e}, video_id 사용")
            return video_id

    def get_playlist_name_from_sheet(self, sheet_url: str, sheet_name: str, row: int) -> str:
        """시트의 특정 행에서 재생목록 이름 추출 (API 호출 최적화: 행 전체를 한 번에 읽기)"""
        # 캐시 확인
        cache_key = (sheet_url, sheet_name, row)
        if cache_key in self._playlist_name_cache:
            logger.debug(f"📦 캐시에서 재생목록 이름 반환 (행 {row})")
            return self._playlist_name_cache[cache_key]

        try:
            def _get_playlist_name():
                workbook = self.client.open_by_url(sheet_url)
                sheet = workbook.worksheet(sheet_name)
                ...
                for header, col_idx in column_mapping.items():
                    if '재생목록' in header and '이름' in header:
                        playlist_col = col_idx
                        break

                # ✅ API 호출 최적화: 행 전체를 한 번에 읽기
                row_values = sheet.row_values(row)
                logger.debug(f"📄 행 {row} 전체 데이터 읽기 완료 ({len(row_values)}개 셀)")

                if not playlist_col:
                    # 재생목록 이름 헤더가 없을 때: 분야1 기반 폴더 구조 사용
                    logger.info(f"⚠️ '재생목록 이름' 열을 찾을 수 없습니다. 분야1 기반 폴더 구조 사용")

                    # 분야1 열 찾기
                    field1_col = column_mapping.get('분야1')
                    if field1_col and field1_col <= len(row_values):
                        field1_value = row_values[field1_col - 1]
                        if field1_value and str(field1_value).strip():
                            logger.debug(f"📁 {row}행에서 분야1 추출: '{field1_value.strip()}'")
                            return str(field1_value).strip()

                    logger.warning(f"⚠️ 분야1도 찾을 수 없거나 비어있습니다. 기본값 사용")
                    return "긴대본_기타"

                # 해당 행의 재생목록 이름 가져오기
                if playlist_col <= len(row_values):
                    playlist_name = row_values[playlist_col - 1]
                    ...
                    return str(playlist_name).strip()
                else:
                    logger.warning(f"⚠️ {row}행의 재생목록 이름 열이 범위를 벗어났습니다. 기본값 사용")
                    return "긴대본_기타"

            # API 호출 with retry
            result = self._api_call_with_retry(_get_playlist_name)

            # 캐시에 저장
            self._playlist_name_cache[cache_key] = result
            return result

        except Exception as e:
            logger.error(f"❌ {row}행 재생목록 이름 추출 실패: {e}")
            # 캐시에 기본값 저장
            self._playlist_name_cache[cache_key] = "긴대본_기타"
            return "긴대본_기타"

    def update_missing_info(self, sheet_url: str, sheet_name: str) -> Dict[str, int]:
        """
        누락정보 업데이트:
        - 대본유무가 'ㅇ'인 행 중에서
        - 대본파일 ID 또는 대본파일 경로가 비어있는 행을 찾아
        - 드라이브에서 파일 검색하여 링크하거나, 없으면 파일 생성
        """
        try:
            logger.info(f"🔍 누락정보 업데이트 시작: {sheet_name}")
            ...
            transcript_status_col = find_col('대본유무')  # 'ㅇ' 확인용
            transcript_content_col = find_col('대본내용')  # 대본 내용 확인
            docs_id_col = find_col('대본파일 ID')
            docs_path_col = find_col('대본파일 경로')
            title_col = find_col('제목')
            video_id_col = find_col('영상ID')
            playlist_col = find_col('분야1(재생목록)')

            if not all([transcript_status_col, title_col]):
                raise ValueError("필수 열을 찾을 수 없습니다 (대본유무, 제목)")

            logger.info(f"📋 열 인덱스: 대본유무={transcript_status_col}, 제목={title_col}, "
                       f"대본파일ID={docs_id_col}, 대본파일경로={docs_path_col}")
            ...
            # 누락 행 찾기
            missing_rows = []
            for row_num in range(10, len(all_values) + 1):  # 10행부터 시작
                row_values = all_values[row_num - 1] if row_num <= len(all_values) else []

                # 대본유무가 'ㅇ'인지 확인
                transcript_status = row_values[transcript_status_col - 1] if len(row_values) >= transcript_status_col else ''
                if transcript_status != 'ㅇ':
                    continue

                # 대본내용이 있는지 확인 (빈 행 제외)
                transcript_content = row_values[transcript_content_col - 1] if transcript_content_col and len(row_values) >= transcript_content_col else ''
                if not transcript_content or transcript_content.strip() == '':
                    continue

                # 대본파일 ID 또는 경로가 비어있는지 확인
                docs_id = row_values[docs_id_col - 1] if docs_id_col and len(row_values) >= docs_id_col else ''
                docs_path = row_values[docs_path_col - 1] if docs_path_col and len(row_values) >= docs_path_col else ''

                if not docs_id or not docs_path:
                    # 제목과 비디오 ID 가져오기
                    title = row_values[title_col - 1] if len(row_values) >= title_col else ''
                    video_id = row_values[video_id_col - 1] if video_id_col and len(row_values) >= video_id_col else ''
                    playlist_name = row_values[playlist_col - 1] if playlist_col and len(row_values) >= playlist_col else '기타'
                    ...
                    missing_rows.append({
                        ...
                        'playlist_name': playlist_name,
                        'transcript_content': transcript_content,
                        'has_docs_id': bool(docs_id),
                        'has_docs_path': bool(docs_path)
                    })

            logger.info(f"📊 누락정보 발견: {len(missing_rows)}개 행")
            ...
            # 기본 폴더 ID (재생목록별 대본 폴더)
            base_folder_id = "1qJy-LUcKdnTw0wgMOj-o26wsgb-ZY8oA"

            # 스프레드시트 이름 가져오기
            spreadsheet_name = workbook.title

            for idx, row_data in enumerate(missing_rows, 1):
                logger.info(f"🔄 {idx}/{len(missing_rows)}: {row_data['row_number']}행 처리 중 - '{row_data['title'][:30]}...'")

                try:
                    # 안전한 파일명 생성
                    safe_title = "".join(c for c in row_data['title'] if c.isalnum() or c in (' ', '-', '_')).rstrip()
                    safe_title = safe_title[:100] if safe_title else row_data.get('video_id', 'untitled')

                    # 폴더 구조 찾기/생성
                    spreadsheet_folder_id = self.find_or_create_folder(spreadsheet_name, base_folder_id)
                    playlist_folder_id = self.find_or_create_folder(row_data['playlist_name'], spreadsheet_folder_id)

                    # 드라이브에서 기존 파일 검색
                    # 1. 제목으로 검색
                    # 2. 'ㅇ제목'으로 검색
                    search_titles = [safe_title, f"ㅇ{safe_title}"]

                    existing_docs = None
                    for search_title in search_titles:
                        existing_docs = self.find_existing_file(
                            search_title,
                            playlist_folder_id,
                            'application/vnd.google-apps.document'
                        )
                        if existing_docs:
                            logger.info(f"📁 기존 파일 발견: '{search_title}'")
                            break
                    ...
                    if existing_docs:
                        ...
                        docs_url = f"https://docs.google.com/document/d/{docs_id}/edit"
                        linked_count += 1
                        logger.info(f"🔗 기존 파일 링크: {docs_id}")
                    else:
                        # 새 파일 생성
                        logger.info(f"📝 새 파일 생성: '{safe_title}'")

                        # 메타데이터 수집
                        metadata = {}
                        if row_data.get('video_id'):
                            try:
                                metadata = self.get_video_metadata_from_sheet(
                                    sheet_url, sheet_name, row_data['row_number'], row_data['video_id']
                                )
                            except Exception as meta_error:
                                logger.warning(f"⚠️ 메타데이터 추출 실패: {meta_error}")
                        ...
                        if row_data.get('video_id'):
                            try:
                                thumbnail_folder_id = self.find_or_create_folder("썸네일", playlist_folder_id)
                                thumbnail_info = self.download_and_save_thumbnail(
                                    row_data['video_id'], row_data['title'], thumbnail_folder_id
                                )
                            except Exception as thumb_error:
                                logger.warning(f"⚠️ 썸네일 다운로드 실패: {thumb_error}")

                        # Google Docs 생성
                        docs_id = self.create_google_doc(
                            title=safe_title,
                            content=row_data['transcript_content'],
                            folder_id=playlist_folder_id,
                        ...
                        docs_url = f"https://docs.google.com/document/d/{docs_id}/edit"
                        created_count += 1
                        logger.info(f"✅ 새 파일 생성 완료: {docs_id}")

                    # 시트 업데이트
                    updates = []
                    if docs_id_col and not row_data['has_docs_id']:
                        updates.append({'col': docs_id_col, 'value': docs_id})
                    if docs_path_col and not row_data['has_docs_path']:
                        updates.append({'col': docs_path_col, 'value': docs_url})
                    ...
                    error_count += 1
                    processed_count += 1
                    logger.error(f"❌ {row_data['row_number']}행 처리 실패: {e}")

            logger.info(f"✅ 누락정보 업데이트 완료: {processed_count}개 처리 (성공: {success_count}, 실패: {error_count})")
        ...
        try:
            ...
            for idx, data in enumerate(extraction_data, 1):
                ...
                if save_location == 'drive':
                    ...
                    # 스프레드시트 이름을 metadata에 추가
                    if not metadata.get('spreadsheet_name'):
                        workbook = self.client.open_by_url(sheet_url)
                        metadata['spreadsheet_name'] = workbook.title
                    ...
                    if '영상 리스트' in sheet_name and metadata.get('channel_name'):
                        ...
                        # 분야1(재생목록) 폴더 찾기/생성
                        logger.debug(f"📁 분야1 폴더 '{data['playlist_name']}' 찾기/생성")
                        playlist_folder_id = self.find_or_create_folder(data['playlist_name'], spreadsheet_folder_id)
                    ...
                    if data.get('video_id'):
                        try:
                            # 썸네일 폴더가 정의되어 있으면 사용, 없으면 playlist_folder_id 사용
                            thumb_folder = thumbnail_folder_id if 'thumbnail_folder_id' in locals() else playlist_folder_id
                            thumbnail_info = self.download_and_save_thumbnail(data['video_id'], data['title'], thumb_folder)
                        except Exception as thumb_error:
                ...
                # Docs 파일 생성 (썸네일 모드나 쇼츠 썸네일 모드가 아닌 경우)
                    ...
                    try:
                        ...
                        # Docs 시트 업데이트 (구글닥스 여부는 업데이트하지 않음)
                        # 대본파일 ID/경로만 업데이트
                        updates = []
                        if data['docs_id_col']:
                            updates.append({'col': data['docs_id_col'], 'value': docs_id})
                        if data['docs_path_col']:
                            updates.append({'col': data['docs_path_col'], 'value': docs_url})
                ...
                if data['needs_txt'] and extraction_mode != "thumbnail":
                    try:
                        if save_location == 'local':
                            ...
                            # TXT 폴더 결정 (이미 위에서 생성된 txt_folder_id 사용)
                            # txt_folder_id는 채널별 또는 재생목록별로 이미 생성되어 있음
                            if 'txt_folder_id' not in locals():
                                # 혹시 없으면 생성
                                txt_folder_id = self.find_or_create_folder("txt", playlist_folder_id)
    ...
    def update_sheet_multiple_columns(self, sheet_url: str, sheet_name: str, row_number: int, updates: List[Dict]):
        """시트의 여러 열을 한번에 업데이트 (전역함수 헤더 보호)"""
        try:
            workbook = self.client.open_by_url(sheet_url)
            sheet = workbook.worksheet(sheet_name)

            # 전역함수 헤더 확인 (9행만 확인)
            headers = sheet.row_values(1) if row_number != 9 else None
            ...
            for update in updates:
                if update and update.get('col') and update.get('value') is not None:
    ...
    def process_bulk_shorts_thumbnail(self, sheet_url: str, sheet_name: str, extraction_data: List[Dict], stop_callback=None) -> Dict[str, int]:
        """쇼츠용 썸네일 10개 단위 순차 벌크 처리 (안정성 강화)"""
        try:
            logger.info(f"🚀 10개 단위 순차 벌크 썸네일 처리 시작: {len(extraction_data)}개 항목")
            ...
            logger.info(f"🎉 10개 단위 순차 벌크 처리 완료: 총 성공 {total_success_count}개, 총 실패 {total_error_count}개")
            return {
                'processed': len(extraction_data),
            ...
            logger.error(f"10개 단위 순차 벌크 처리 중 오류 발생: {e}")
            return {
                'processed': 0,
                'docs_success': 0,
                'txt_success': 0,
    ...
    def process_bulk_docs_extraction(self, sheet_url: str, sheet_name: str, extraction_data: List[Dict], 
        ...
        try:
            ...
            # 10개 이상인 경우 10개씩 분할
            if len(extraction_data) >= 10:
                max_batch_size = 10
                logger.info(f"📦 10개 이상 감지: 최대 배치 크기를 {max_batch_size}개로 제한")
            else:
    ...
    def update_recent_30_avg_views_only(self, spreadsheet_url, video_sheet_name, progress_callback=None):
        ...
        try:
            ...
            recent_30_median_views_field = '최근 30개 영상 중위 조회수'
            collected_avg_views_field = '수집한 영상 평균 조회수'
            collected_video_count_field = '수집영상 갯수'
            avg_duration_field = '평균 영상길이'
            ...
            channel_collected_avg_views_col = self.find_header_column(channel_headers, collected_avg_views_field, allow_partial=False)
            channel_collected_video_count_col = self.find_header_column(channel_headers, collected_video_count_field, allow_partial=False)
            channel_avg_duration_col = self.find_header_column(channel_headers, avg_duration_field, allow_partial=False)
            ...
            if channel_collected_avg_views_col:
                logger.info(f"✅ 채널 리스트 수집한 영상 평균 조회수 열: {channel_collected_avg_views_col}")
            if channel_collected_video_count_col:
                logger.info(f"✅ 채널 리스트 수집영상 갯수 열: {channel_collected_video_count_col}")
            if channel_avg_duration_col:
                logger.info(f"✅ 채널 리스트 평균 영상길이 열: {channel_avg_duration_col}")
            ...
            video_duration_col = self.find_header_column(video_headers, '영상길이', allow_partial=False)
            ...
            from collections import defaultdict
            channel_recent_videos = defaultdict(list)  # 최근 30개 계산용: (업로드날짜, 조회수)
            channel_all_views = defaultdict(list)      # 수집한 영상 평균 조회수용: 모든 조회수
            channel_all_durations = defaultdict(list)  # 평균 영상길이용: 모든 영상길이(초)
            ...
            for vid_row_idx, vid_row in enumerate(video_data[9:], 10):
                ...
                if views_str:
                    view_count = self.parse_view_count(views_str)
                    if view_count > 0:
                        # 수집한 영상 평균 조회수용 (모든 영상)
                        channel_all_views[vid_channel_id].append(view_count)

                        # 최근 30개 영상용 (업로드날짜가 있는 경우)
                        if upload_date:
                            channel_recent_videos[vid_channel_id].append((upload_date, view_count))
                ...
                if video_duration_col and len(vid_row) >= video_duration_col:
                    ...
                    if duration_str:
                        duration_seconds = self.parse_video_duration_to_seconds(duration_str)
                        if duration_seconds > 0:
                            channel_all_durations[vid_channel_id].append(duration_seconds)

            logger.info(f"✅ {len(channel_recent_videos)}개 채널의 최근 영상 데이터 수집 완료 (업로드날짜+조회수)")
            logger.info(f"✅ {len(channel_all_views)}개 채널의 수집한 영상 조회수 데이터 수집 완료")
            logger.info(f"✅ {len(channel_all_durations)}개 채널의 영상길이 데이터 수집 완료")
            ...
            for ch_row_idx, ch_row in enumerate(channel_data[9:], 10):
                ...
                # 수집한 영상 평균 조회수 업데이트
                if channel_collected_avg_views_col and channel_id in channel_all_views and len(channel_all_views[channel_id]) > 0:
                    total_views = sum(channel_all_views[channel_id])
                    avg_collected_views = total_views // len(channel_all_views[channel_id])
                    avg_collected_views_text = self.format_number_with_comma(avg_collected_views)
                    channel_updates.append({
                        'range': f'{self._col_num_to_letter(channel_collected_avg_views_col)}{ch_row_idx}',
                        'values': [[avg_collected_views_text]]
                    })
                    logger.info(f"📊 채널 '{channel_id}' 수집한 영상 평균 조회수: {avg_collected_views_text} (총 {self.format_number_with_comma(total_views)} / {len(channel_all_views[channel_id])}개 영상)")

                # 수집영상 갯수 업데이트
                if channel_collected_video_count_col and channel_id in channel_all_views:
                    video_count = len(channel_all_views[channel_id])
                    video_count_text = self.format_number_with_comma(video_count)
                    channel_updates.append({
                        'range': f'{self._col_num_to_letter(channel_collected_video_count_col)}{ch_row_idx}',
                        'values': [[video_count_text]]
                    })
                    logger.info(f"📊 채널 '{channel_id}' 수집영상 갯수: {video_count_text}개")

                # 평균 영상길이 업데이트
                if channel_avg_duration_col and channel_id in channel_all_durations and len(channel_all_durations[channel_id]) > 0:
                    avg_duration_seconds = sum(channel_all_durations[channel_id]) // len(channel_all_durations[channel_id])
                    avg_duration_text = self.seconds_to_duration_text(avg_duration_seconds)
                    if avg_duration_text:
                        channel_updates.append({
                            'range': f'{self._col_num_to_letter(channel_avg_duration_col)}{ch_row_idx}',
                        ...
                        logger.info(f"📊 채널 '{channel_id}' 평균 영상길이: {avg_duration_text} ({len(channel_all_durations[channel_id])}개 영상 기준)")
            ...
            if channel_updates or video_updates:
                ...
                result_msg = f"✅ 채널 리스트 업데이트 완료!\n\n" \
                           ...
                           f"  - 수집영상 갯수\n" \
                           f"  - 평균 영상길이\n" \
                           f"  - 벤치마킹 채널여부 (영상 시트 동기화)\n\n" \
                           f"• 채널 리스트 업데이트 셀: {len(channel_updates)}개\n" \
                           f"• 영상 시트 업데이트 셀: {len(video_updates)}개"
    ...
    def update_longform_avg_views_only(self, spreadsheet_url, video_sheet_name, progress_callback=None):
        """
        채널 리스트의 롱폼 전용 평균 조회수 열들을 업데이트 (숏폼 제외):
        - 롱폼 최근 30개 영상 평균 조회수
        - 롱폼 최근 30개 영상 중위 조회수
        - 롱폼 수집한 영상 평균 조회수
        - 롱폼 수집영상 갯수
        ...
        """
        try:
            logger.info(f"🔄 롱폼 전용 채널 리스트 업데이트 시작 (영상 시트: {video_sheet_name})")
            ...
            # 롱폼 전용 필드명 정의 (기존 헤더와 동일한 이름 사용)
            longform_recent_30_avg_views_field = '최근 30개 영상 평균 조회수'
            longform_recent_30_median_views_field = '최근 30개 영상 중위 조회수'
            longform_collected_avg_views_field = '수집한 영상 평균 조회수'
            longform_collected_video_count_field = '수집영상 갯수'
            longform_avg_duration_field = '평균 영상길이'
            ...
            # 채널 리스트에서 필요한 열 찾기 (완전 일치만)
            channel_longform_recent_30_avg_views_col = self.find_header_column(channel_headers, longform_recent_30_avg_views_field, allow_partial=False)
            channel_longform_recent_30_median_views_col = self.find_header_column(channel_headers, longform_recent_30_median_views_field, allow_partial=False)
            channel_longform_collected_avg_views_col = self.find_header_column(channel_headers, longform_collected_avg_views_field, allow_partial=False)
            channel_longform_collected_video_count_col = self.find_header_column(channel_headers, longform_collected_video_count_field, allow_partial=False)
            channel_longform_avg_duration_col = self.find_header_column(channel_headers, longform_avg_duration_field, allow_partial=False)
            channel_id_col = self.find_header_column(channel_headers, '채널 ID', allow_partial=False)
            channel_benchmarking_col = self.find_header_column(channel_headers, '벤치마킹 채널여부', allow_partial=False)
            ...
            # 찾은 열 로깅
            if channel_longform_recent_30_avg_views_col:
                logger.info(f"✅ 채널 리스트 최근 30개 영상 평균 조회수 열: {channel_longform_recent_30_avg_views_col}")
            if channel_longform_recent_30_median_views_col:
                logger.info(f"✅ 채널 리스트 최근 30개 영상 중위 조회수 열: {channel_longform_recent_30_median_views_col}")
            if channel_longform_collected_avg_views_col:
                logger.info(f"✅ 채널 리스트 수집한 영상 평균 조회수 열: {channel_longform_collected_avg_views_col}")
            if channel_longform_collected_video_count_col:
                logger.info(f"✅ 채널 리스트 수집영상 갯수 열: {channel_longform_collected_video_count_col}")
            if channel_longform_avg_duration_col:
                logger.info(f"✅ 채널 리스트 평균 영상길이 열: {channel_longform_avg_duration_col}")
            logger.info(f"✅ 채널 리스트 채널 ID 열: {channel_id_col}")
            ...
            video_views_col = self.find_header_column(video_headers, '조회수', allow_partial=False)
            video_duration_col = self.find_header_column(video_headers, '영상길이', allow_partial=False)
            video_shortform_col = self.find_header_column(video_headers, '숏폼여부', allow_partial=False)
            ...
            logger.info(f"📌 영상 시트 - 채널ID: {video_channel_id_col}열, 업로드날짜: {video_upload_date_col}열, 조회수: {video_views_col}열, 숏폼여부: {video_shortform_col}열")

            if not video_channel_id_col or not video_views_col:
                missing = []
                if not video_channel_id_col:
                    missing.append("채널 ID")
                if not video_views_col:
            ...
            if not video_shortform_col:
                logger.warning(f"⚠️ 영상 시트에서 '숏폼여부' 열을 찾을 수 없습니다. 모든 영상을 롱폼으로 간주합니다.")
            ...
            # 숏폼 여부 판단 함수
            def is_shortform(shortform_value):
                """숏폼 여부 판단 (숏폼이면 True, 롱폼이면 False)"""
                if not shortform_value:
                    return False
                value_str = str(shortform_value).strip().lower()
                # 'o', 'ㅇ', 'yes', '1', 'true', '숏폼' 등을 숏폼으로 간주
                return value_str in ['o', 'ㅇ', 'yes', '1', 'true', '숏폼', 'short']

            # 채널별 롱폼 영상 데이터 수집
            from collections import defaultdict
            channel_longform_recent_videos = defaultdict(list)  # 최근 30개 계산용: (업로드날짜, 조회수)
            channel_longform_all_views = defaultdict(list)      # 수집한 영상 평균 조회수용: 모든 조회수
            channel_longform_all_durations = defaultdict(list)  # 평균 영상길이용: 모든 영상길이(초)

            for vid_row_idx, vid_row in enumerate(video_data[9:], 10):
                if len(vid_row) < max(video_channel_id_col, video_upload_date_col or 0, video_views_col):
                    continue
                ...
                # 숏폼 여부 체크 - 숏폼이면 스킵
                if video_shortform_col and len(vid_row) >= video_shortform_col:
                    shortform_value = vid_row[video_shortform_col - 1]
                    if is_shortform(shortform_value):
                        continue  # 숏폼은 건너뜀

                # 업로드날짜 수집 (최근 30개 영상용)
                upload_date_raw = vid_row[video_upload_date_col - 1] if video_upload_date_col and len(vid_row) >= video_upload_date_col else None
                upload_date = parse_date(upload_date_raw)
                ...
                if views_str:
                    view_count = self.parse_view_count(views_str)
                    if view_count > 0:
                        # 수집한 영상 평균 조회수용 (모든 롱폼 영상)
                        channel_longform_all_views[vid_channel_id].append(view_count)

                        # 최근 30개 영상용 (업로드날짜가 있는 경우)
                        if upload_date:
                            channel_longform_recent_videos[vid_channel_id].append((upload_date, view_count))
                ...
                if video_duration_col and len(vid_row) >= video_duration_col:
                    ...
                    if duration_str:
                        duration_seconds = self.parse_video_duration_to_seconds(duration_str)
                        if duration_seconds > 0:
                            channel_longform_all_durations[vid_channel_id].append(duration_seconds)

            logger.info(f"✅ {len(channel_longform_recent_videos)}개 채널의 롱폼 최근 영상 데이터 수집 완료 (업로드날짜+조회수)")
            logger.info(f"✅ {len(channel_longform_all_views)}개 채널의 롱폼 수집한 영상 조회수 데이터 수집 완료")
            logger.info(f"✅ {len(channel_longform_all_durations)}개 채널의 롱폼 영상길이 데이터 수집 완료")
            ...
            for ch_row_idx, ch_row in enumerate(channel_data[9:], 10):
                ...
                # 롱폼 최근 30개 영상 평균 조회수 및 중위 조회수 계산
                if channel_longform_recent_30_avg_views_col and channel_id in channel_longform_recent_videos and len(channel_longform_recent_videos[channel_id]) > 0:
                    # 업로드날짜 기준으로 내림차순 정렬 (최신순)
                    sorted_videos = sorted(channel_longform_recent_videos[channel_id], key=lambda x: x[0], reverse=True)
                    # 최근 30개 선택
                    recent_30 = sorted_videos[:30]
                    # 조회수 평균 계산
                    recent_30_views = [views for _, views in recent_30]
                    ...
                    # 평균 조회수 업데이트
                    channel_updates.append({
                        'range': f'{self._col_num_to_letter(channel_longform_recent_30_avg_views_col)}{ch_row_idx}',
                        'values': [[avg_recent_30_text]]
                    })
                    logger.info(f"📊 채널 '{channel_id}' 롱폼 최근 30개 영상 평균 조회수: {avg_recent_30_text} ({len(recent_30)}개 영상 기준)")

                    # 중위 조회수 계산 (상위 20%, 하위 20% 제외)
                    if channel_longform_recent_30_median_views_col and len(recent_30_views) >= 5:
                        # 조회수 기준 정렬
                        sorted_views = sorted(recent_30_views)
                        total_count = len(sorted_views)
                        ...
                        if trimmed_views:
                            ...
                            channel_updates.append({
                                'range': f'{self._col_num_to_letter(channel_longform_recent_30_median_views_col)}{ch_row_idx}',
                                'values': [[median_views_text]]
                            })
                            logger.info(f"📊 채널 '{channel_id}' 롱폼 최근 30개 영상 중위 조회수: {median_views_text} (상위/하위 {remove_count}개씩 제외, {len(trimmed_views)}개 영상 기준)")

                # 롱폼 수집한 영상 평균 조회수 업데이트
                if channel_longform_collected_avg_views_col and channel_id in channel_longform_all_views and len(channel_longform_all_views[channel_id]) > 0:
                    total_views = sum(channel_longform_all_views[channel_id])
                    avg_collected_views = total_views // len(channel_longform_all_views[channel_id])
                    avg_collected_views_text = self.format_number_with_comma(avg_collected_views)
                    channel_updates.append({
                        'range': f'{self._col_num_to_letter(channel_longform_collected_avg_views_col)}{ch_row_idx}',
                        'values': [[avg_collected_views_text]]
                    })
                    logger.info(f"📊 채널 '{channel_id}' 롱폼 수집한 영상 평균 조회수: {avg_collected_views_text} (총 {self.format_number_with_comma(total_views)} / {len(channel_longform_all_views[channel_id])}개 영상)")

                # 롱폼 수집영상 갯수 업데이트
                if channel_longform_collected_video_count_col and channel_id in channel_longform_all_views:
                    video_count = len(channel_longform_all_views[channel_id])
                    video_count_text = self.format_number_with_comma(video_count)
                    channel_updates.append({
                        'range': f'{self._col_num_to_letter(channel_longform_collected_video_count_col)}{ch_row_idx}',
                        'values': [[video_count_text]]
                    })
                    logger.info(f"📊 채널 '{channel_id}' 롱폼 수집영상 갯수: {video_count_text}개")

                # 롱폼 평균 영상길이 업데이트
                if channel_longform_avg_duration_col and channel_id in channel_longform_all_durations and len(channel_longform_all_durations[channel_id]) > 0:
                    avg_duration_seconds = sum(channel_longform_all_durations[channel_id]) // len(channel_longform_all_durations[channel_id])
                    avg_duration_text = self.seconds_to_duration_text(avg_duration_seconds)
                    if avg_duration_text:
                        channel_updates.append({
                            'range': f'{self._col_num_to_letter(channel_longform_avg_duration_col)}{ch_row_idx}',
                            'values': [[avg_duration_text]]
                        })
                        logger.info(f"📊 채널 '{channel_id}' 롱폼 평균 영상길이: {avg_duration_text} ({len(channel_longform_all_durations[channel_id])}개 영상 기준)")

            # 배치 업데이트 실행
            if channel_updates:
                total_updates = len(channel_updates)
                if progress_callback:
                    progress_callback(total_channels, total_channels, f"업데이트 적용 중... ({total_updates}개 셀)")
                ...
                result_msg = f"✅ 롱폼 전용 채널 리스트 업데이트 완료!\n\n" \
                           f"• 처리 채널: {processed_channels}개\n" \
                           f"• 업데이트된 정보:\n" \
                           f"  - 롱폼 최근 30개 영상 평균 조회수\n" \
                           f"  - 롱폼 최근 30개 영상 중위 조회수\n" \
                           f"  - 롱폼 수집한 영상 평균 조회수\n" \
                           f"  - 롱폼 수집영상 갯수\n" \
                           f"  - 롱폼 평균 영상길이\n\n" \
                           f"• 채널 리스트 업데이트 셀: {len(channel_updates)}개"
                ...
                logger.info(f"⚠️ 업데이트할 데이터가 없습니다")
                return "⚠️ 업데이트할 데이터가 없습니다.\n롱폼 영상 데이터가 있는 채널이 없습니다."

        except Exception as e:
            logger.error(f"❌ 롱폼 전용 채널 리스트 업데이트 실패: {str(e)}")
            raise
...
class TranscriptExtractorGUI:
    def __init__(self, root):
        ...
        self.current_sheet_url = self.sheets_manager.available_spreadsheets["채널수집_2.국내롱폼"]
    ...
    def setup_ui(self):
        ...
        # 제목
        title_label = ttk.Label(main_frame, text="유튜브 롱폼 대본 추출기",
                               font=('Arial', 16, 'bold'))
        ...
        self.spreadsheet_combo.set("채널수집_2.국내롱폼")
    ...
    def setup_transcript_section(self, parent):
        ...
        # 누락정보 업데이트 버튼
        self.update_missing_button = ttk.Button(transcript_frame, text="누락정보 업데이트",
                                               command=self.start_missing_info_update)
        self.update_missing_button.grid(row=4, column=0, pady=(5, 0), sticky=(tk.W, tk.E))

        # 구글 닥스 섹션
        docs_frame = ttk.LabelFrame(transcript_frame, text="구글 닥스 추출", padding="10")
        docs_frame.grid(row=5, column=0, sticky=(tk.W, tk.E), pady=(15, 0))
        ...
        self.extraction_mode_var = tk.StringVar(value="docs_only")  # 기본값: Docs만 추출

        # Docs만 추출 라디오버튼
        self.docs_only_radio = ttk.Radiobutton(
            mode_frame,
            text="Docs만 추출 (기본)",
        ...
        self.docs_only_radio.grid(row=0, column=0, sticky=tk.W, pady=(0, 5))
        ...
        self.both_radio.grid(row=1, column=0, sticky=tk.W, pady=(0, 5))
        ...
        self.missing_radio.grid(row=2, column=0, sticky=tk.W)
        ...
        self.thumbnail_radio.grid(row=3, column=0, sticky=tk.W)
        ...
        self.shorts_thumbnail_radio.grid(row=4, column=0, sticky=tk.W)
        
        # 일반 벌크 처리 체크박스 (모든 모드 공통)
        self.bulk_docs_var = tk.BooleanVar(value=False)
        self.bulk_docs_check = ttk.Checkbutton(
            mode_frame,
            text="벌크 처리 (10개 이상일 때 10개씩 순차 처리)",
            variable=self.bulk_docs_var
        )
        self.bulk_docs_check.grid(row=5, column=0, sticky=tk.W, pady=(5, 0))
        
        # 썸네일 전용 벌크 처리 체크박스
        self.bulk_thumbnail_var = tk.BooleanVar(value=False)
        self.bulk_thumbnail_check = ttk.Checkbutton(
            mode_frame,
            text="썸네일 10개 단위 순차 벌크 처리 (타임아웃 방지 + API 최적화)",
            variable=self.bulk_thumbnail_var
        )
        self.bulk_thumbnail_check.grid(row=6, column=0, sticky=tk.W, padx=(20, 0))
        ...
        # 채널 시트 평균 조회수 업데이트 버튼
        self.update_recent_30_views_button = ttk.Button(button_frame,
                                                        text="채널 시트 평균 조회수 업데이트",
                                                        command=self.update_recent_30_avg_views)
        self.update_recent_30_views_button.grid(row=0, column=2, padx=(0, 5))

        # 롱폼전용 평균 조회수 업데이트 버튼
        self.update_longform_views_button = ttk.Button(button_frame,
                                                       text="롱폼전용 평균 조회수 업데이트",
                                                       command=self.update_longform_avg_views)
        self.update_longform_views_button.grid(row=1, column=2, padx=(0, 5), pady=(5, 0))
        ...
        desc_label = ttk.Label(channel_video_update_frame,
                              text="[채널 리스트 업데이트] 채널 리스트 ↔ 영상 시트 양방향 동기화\n"
                                   ...
                                   "[채널 시트 평균 조회수 업데이트] 채널 리스트의 여러 평균 조회수 열 업데이트\n"
                                   "• 최근 30개 영상 평균 조회수, 수집한 영상 평균 조회수, 평균 영상길이 등 계산\n"
                                   "• 영상 시트의 모든 영상(숏폼+롱폼) 데이터 기준으로 계산\n\n"
                                   "[롱폼전용 평균 조회수 업데이트] 채널 리스트의 롱폼 전용 평균 조회수 열 업데이트\n"
                                   "• 영상 시트의 '숏폼여부' 열이 숏폼이 아닌 영상들만 기준으로 계산\n"
                                   "• 롱폼 최근 30개 평균, 롱폼 수집한 영상 평균, 롱폼 평균 영상길이 등\n\n"
    ...
    def update_sheet_list(self, sheet_names):
        ...
        # 자동 선택 제거: 사용자가 직접 시트를 선택하도록 함
        if sheet_names:
            logger.info(f"시트 목록 업데이트 완료: {len(sheet_names)}개 시트 발견")
        self.sheet_info_var.set("시트 목록이 업데이트되었습니다. 시트를 선택해주세요.")
        ...
        # 인증이 완료되고 시트 목록이 로드된 후, 사용 레퍼런스 영상 시트 자동 로드 취소
...
{'='*50}
    ...
    def start_missing_info_update(self):
        """누락정보 업데이트 시작"""
        if self.is_running:
        ...
        self.is_running = True
        self.update_missing_button.config(state='disabled', text='업데이트 중...')
        self.progress_bar.start()

        # 별도 스레드에서 실행
        thread = threading.Thread(target=self.run_missing_info_update)
        thread.daemon = True
        thread.start()

    def run_missing_info_update(self):
        """실제 누락정보 업데이트 작업 수행"""
        try:
            self.root.after(0, lambda: self.update_progress("구글 시트 인증 중..."))
            ...
            self.root.after(0, lambda: self.update_progress("누락정보 분석 중..."))

            # 누락정보 업데이트 실행
            result = self.sheets_manager.update_missing_info(
                self.current_sheet_url,
                selected_sheet_name
            )

            # 결과 표시
            self.root.after(0, lambda: self.show_missing_update_result(result))
            self.root.after(0, lambda: self.update_progress("누락정보 업데이트 완료"))

        except Exception as e:
            logger.exception("누락정보 업데이트 중 오류 발생:")
            self.root.after(0, lambda: messagebox.showerror("오류", f"누락정보 업데이트 중 오류가 발생했습니다:\n{str(e)}"))
        finally:
            self.root.after(0, self.missing_update_finished)

    def missing_update_finished(self):
        """누락정보 업데이트 완료 후 처리"""
        self.is_running = False
        self.update_missing_button.config(state='normal', text='누락정보 업데이트')
        self.progress_bar.stop()

    def show_missing_update_result(self, result):
        """누락정보 업데이트 결과 표시"""
        result_window = tk.Toplevel(self.root)
        result_window.title("누락정보 업데이트 결과")
...
📍 처리된 행: {result.get('processed_count', 0)}개
✅ 성공: {result.get('success_count', 0)}개
❌ 실패: {result.get('error_count', 0)}개
📁 파일 생성: {result.get('created_count', 0)}개
🔗 파일 링크: {result.get('linked_count', 0)}개
...
{'='*50}
    ...
    def update_recent_30_avg_views(self):
        ...
        if not messagebox.askyesno("채널 시트 평균 조회수 업데이트",
                                  f"채널 리스트의 여러 평균 조회수 열을 업데이트하시겠습니까?\n\n"
                                  f"[처리 대상]\n"
                                  f"• 채널 리스트의 '가져왔는지 여부'가 입력된 모든 채널\n"
                                  f"• '{video_sheet}' 시트의 모든 영상(숏폼+롱폼) 데이터 사용\n\n"
                                  f"[업데이트 항목]\n"
                                  f"• 최근 30개 영상 평균 조회수\n"
                                  f"• 최근 30개 영상 중위 조회수\n"
                                  f"• 수집한 영상 평균 조회수\n"
                                  ...
                                  f"• 평균 영상길이\n\n"
                                  f"[주의사항]\n"
                                  f"• 영상 시트에 '영상 업로드날짜' 열이 없으면 일부 계산 불가"):
            return
        ...
        self.channel_update_progress['value'] = 0
        self.channel_update_status_label.config(text="시작...")
        self.update_progress("채널 시트 평균 조회수 업데이트 중...")
        self.progress_bar.start()
    ...
    def _run_recent_30_views_update(self, video_sheet_name):
        """채널 시트 평균 조회수 업데이트 작업 수행"""
        try:
            self.root.after(0, lambda: self.update_progress("채널 시트 평균 조회수 업데이트 시작..."))
            ...
            error_msg = str(e)
            logger.error(f"채널 시트 평균 조회수 업데이트 실패: {error_msg}")
            self.root.after(0, lambda: self.update_progress(f"❌ 업데이트 실패: {error_msg}"))
            self.root.after(0, lambda: messagebox.showerror("오류", f"채널 시트 평균 조회수 업데이트 중 오류가 발생했습니다:\n{error_msg}"))

        finally:
            self.is_running = False
            self.root.after(0, lambda: self.update_recent_30_views_button.config(state='normal', text='채널 시트 평균 조회수 업데이트'))
            self.root.after(0, lambda: self.channel_update_status_label.config(text=""))
            self.root.after(0, lambda: self.progress_bar.stop())

    def update_longform_avg_views(self):
        """롱폼 전용 평균 조회수 업데이트 (숏폼 제외)"""
        if self.is_running:
            messagebox.showwarning("경고", "다른 작업이 실행 중입니다.")
            return
        ...
        if not messagebox.askyesno("롱폼전용 평균 조회수 업데이트",
                                  f"채널 리스트의 평균 조회수 열을 롱폼 영상만으로 업데이트하시겠습니까?\n\n"
                                  f"[처리 대상]\n"
                                  f"• 채널 리스트의 '가져왔는지 여부'가 입력된 모든 채널\n"
                                  f"• '{video_sheet}' 시트의 '숏폼여부' 열이 숏폼이 아닌 영상만 사용 (롱폼만)\n\n"
                                  f"[업데이트 항목] - 기존 헤더에 롱폼 데이터로 덮어쓰기\n"
                                  f"• 최근 30개 영상 평균 조회수 (롱폼만)\n"
                                  f"• 최근 30개 영상 중위 조회수 (롱폼만)\n"
                                  f"• 수집한 영상 평균 조회수 (롱폼만)\n"
                                  f"• 수집영상 갯수 (롱폼만)\n"
                                  f"• 평균 영상길이 (롱폼만)\n\n"
                                  f"[주의사항]\n"
                                  f"• 영상 시트에 '숏폼여부' 및 '영상 업로드날짜' 열이 필요\n"
                                  f"• 기존 데이터를 롱폼 전용 데이터로 덮어씁니다"):
            return

        # 업데이트 실행
        self.is_running = True
        self.update_longform_views_button.config(state='disabled', text='업데이트 중...')
        self.channel_update_progress['value'] = 0
        self.channel_update_status_label.config(text="시작...")
        self.update_progress("롱폼전용 평균 조회수 업데이트 중...")
        self.progress_bar.start()

        # 별도 스레드에서 실행
        thread = threading.Thread(target=self._run_longform_views_update, args=(video_sheet,))
        thread.daemon = True
        thread.start()

    def _run_longform_views_update(self, video_sheet_name):
        """롱폼 전용 평균 조회수 업데이트 작업 수행"""
        try:
            self.root.after(0, lambda: self.update_progress("롱폼전용 평균 조회수 업데이트 시작..."))

            # 업데이트 실행
            result_msg = self.sheets_manager.update_longform_avg_views_only(
                self.current_sheet_url,
                video_sheet_name,
                progress_callback=self._update_channel_progress
            )
            ...
            error_msg = str(e)
            logger.error(f"롱폼전용 평균 조회수 업데이트 실패: {error_msg}")
            self.root.after(0, lambda: self.update_progress(f"❌ 업데이트 실패: {error_msg}"))
            self.root.after(0, lambda: messagebox.showerror("오류", f"롱폼전용 평균 조회수 업데이트 중 오류가 발생했습니다:\n{error_msg}"))

        finally:
            self.is_running = False
            self.root.after(0, lambda: self.update_longform_views_button.config(state='normal', text='롱폼전용 평균 조회수 업데이트'))
            self.root.after(0, lambda: self.channel_update_status_label.config(text=""))
            self.root.after(0, lambda: self.progress_bar.stop())
        ...
        try:
            ...
            if sort_limit_enabled and sort_limit:
                ...
                if sorted_count == 0:
                    ...
                    target_sheet = self.target_sheet_var.get() if hasattr(self, 'target_sheet_var') else "조건 추출 영상"
    ...
    def _run_extract_ai_reference_data(self):
        ...
        try:
            ...
            required_columns = [
                ...
                '카테고리 ID', '디스크립션', '후킹자막',
                '썸네일 링크', '대본추출 여부', '대본파일 ID', '대본파일 경로',
                '대본내용', '대본 텍스트수'
            ]
    ...
    def _run_conditional_extraction(self, field1_filters, field2_filters, shortform_filters,
        ...
        try:
            ...
            # max_video_upload_days 변수 초기화 (현재 사용하지 않음)
            max_video_upload_days = None
            ...
            if not filtered_data:
                ...
                # 대상 시트에 붙여넣기 (원본 시트 이름 및 대상 시트 이름 전달)
                ...
                if success:
                    self.progress_var.set(f"추출 완료: {len(filtered_data)}개 영상이 '{target_sheet}' 시트에 추가되었습니다.")
                    logger.info(f"🎉 조건부 추출 완료: {len(filtered_data)}개 영상이 '{target_sheet}' 시트에 성공적으로 추출되어 저장되었습니다.")
                    messagebox.showinfo("완료", f"{len(filtered_data)}개의 영상이 '{target_sheet}' 시트에 추가되었습니다.")
                else:
    ...
    def _run_copy_to_ai_spreadsheet(self, source_sheet_name):
        ...
        try:
            ...
            # 2. 대상 스프레드시트 열기
            target_spreadsheet_url = "https://docs.google.com/spreadsheets/d/1MOtlkjQTP0AE0EbAYW56DByqt71Fz-vSJXwc2S4RBoI/edit?usp=drive_link"
