...
class GoogleSheetsManager:
    ...
    def _retry_on_rate_limit(self, func, *args, max_retries=5, **kwargs):
        """
        Google Sheets API Rate Limit 발생 시 재시도 로직 (Exponential Backoff)
        ...
        while retry_count < max_retries:
            try:
                ...
                # Rate Limit 오류 확인 (429 에러)
                # gspread.exceptions.APIError의 response는 dict 또는 Response 객체
                error_code = None

                # response가 dict인 경우
                if isinstance(e.response, dict):
                    error_code = e.response.get('code')
                # response.status_code 속성이 있는 경우
                ...
                if error_code == 429:
                    ...
                    # Rate Limit은 분당 제한이므로 60초 대기 후 재시도
                    delay = 65  # 안전하게 65초 대기
                    print(f"Rate Limit 도달. {delay}초 후 재시도... ({retry_count}/{max_retries})")
                    time.sleep(delay)
                else:
                    # Rate Limit이 아닌 다른 API 오류는 바로 예외 발생
                    raise
            except Exception as e:
                # gspread 외 다른 예외는 바로 발생
                raise

        # 여기까지 도달하지 않지만 안전을 위해
    ...
    def get_or_create_worksheet(self, sheet_name: str) -> gspread.Worksheet:
        ...
        """
        try:
            worksheet = self._retry_on_rate_limit(self.spreadsheet.worksheet, sheet_name)
        except gspread.WorksheetNotFound:
            worksheet = self._retry_on_rate_limit(
    ...
    def get_headers(self, sheet_name: str) -> List[str]:
        ...
        headers = self._retry_on_rate_limit(worksheet.row_values, 1)
    ...
    def get_last_row(self, sheet_name: str, start_row: int = 10) -> int:
        ...
        """
        worksheet = self.get_or_create_worksheet(sheet_name)
        col_a_values = self._retry_on_rate_limit(worksheet.col_values, 1)  # A열 전체 값
    ...
    def bulk_append_data(self, sheet_name: str, data_list: List[Dict[str, any]],
        ...
        if unmatched_keys:
            ...
            if new_headers_added:
                ...
                self._retry_on_rate_limit(worksheet.update, header_range, [new_headers_added], value_input_option='USER_ENTERED')
                time.sleep(0.1)  # API 호출 간 딜레이
    ...
    def _copy_row_format(self, worksheet, source_row: int, target_start_row: int,
        ...
        """
        try:
            # 한 번에 여러 행의 서식을 복사하는 것보다, 한 행씩 복사하는 것이 안전
            # 하지만 API 호출 수를 줄이기 위해 repeatCell을 사용하여 서식 복사

            # 먼저 원본 행의 서식 정보를 가져옴
            # copyPaste를 사용하여 한 번에 모든 대상 행에 복사
            request = {
                "copyPaste": {
                    "source": {
    ...
    def initialize_headers(self, sheet_name: str):
        ...
        worksheet = self.get_or_create_worksheet(sheet_name)
        self._retry_on_rate_limit(worksheet.update, 'A1', [headers], value_input_option='USER_ENTERED')
        time.sleep(0.1)  # API 호출 간 딜레이
    ...
    def get_playlist_data(self, sheet_name: str = '재생목록ID') -> List[Dict]:
        ...
        # 디버깅: 헤더 출력
        print(f"[디버그] 재생목록ID 시트 헤더: {headers}")
        ...
        for idx, header in enumerate(headers):
            ...
            # 재생목록 ID 컬럼 (다양한 형태 지원)
            if normalized in ['재생목록ID', '재생목록아이디', '재생목록id', 'playlistid', 'playlist_id']:
                col_mapping['playlist_id'] = idx + 1
            # 재생목록 이름 컬럼
            elif normalized in ['재생목록이름', '재생목록명', '이름', 'playlistname', 'playlist_name', '제목']:
                col_mapping['playlist_name'] = idx + 1
            # 영상 개수 컬럼
            elif normalized in ['영상갯수', '영상개수', '영상수', 'videocount', 'video_count']:
                col_mapping['video_count'] = idx + 1
            # 마지막 체크일 컬럼
            elif normalized in ['마지막체크일', '마지막확인일', '체크일', 'lastcheck', 'last_check']:
                col_mapping['last_check'] = idx + 1

        print(f"[디버그] 컬럼 매핑: {col_mapping}")
        ...
        # 모든 데이터 가져오기 (2행부터)
        all_values = self._retry_on_rate_limit(worksheet.get_all_values)
        playlist_data = []
        ...
        for row_idx, row in enumerate(all_values[1:], start=2):  # 2행부터 시작
            ...
            if playlist_id_col > 0 and len(row) >= playlist_id_col and row[playlist_id_col - 1]:
                playlist_name = ''
                if 'playlist_name' in col_mapping and len(row) >= col_mapping['playlist_name']:
                    playlist_name = row[col_mapping['playlist_name'] - 1].strip()

                playlist_id_value = row[col_mapping['playlist_id'] - 1] if 'playlist_id' in col_mapping and len(row) >= col_mapping['playlist_id'] else ''
                print(f"[디버그] 행 {row_idx}: ID={playlist_id_value}, 이름={playlist_name}")
    ...
    def update_channel_info(self, sheet_name: str, row: int, channel_info: Dict):
        ...
        # 배치 업데이트
        if updates:
            self._retry_on_rate_limit(worksheet.batch_update, updates, value_input_option='USER_ENTERED')
            time.sleep(0.1)  # API 호출 간 딜레이
    ...
    def count_videos_in_video_sheet(self, channel_id: str, sheet_name: str = '영상 리스트') -> int:
        ...
        channel_id_col = None
        for idx, header in enumerate(headers):
            if normalize_header(header) == '채널ID':
                channel_id_col = idx
                break
    ...
    def update_playlist_info(self, sheet_name: str, row: int, video_count: int):
        ...
        # 헤더에서 컬럼 인덱스 찾기 (없으면 무시)
        try:
            video_count_col = headers.index('영상갯수') + 1
            # 영상갯수 업데이트
            video_count_cell = f'{self._col_num_to_letter(video_count_col)}{row}'
            self._retry_on_rate_limit(worksheet.update, video_count_cell, [[video_count]], value_input_option='USER_ENTERED')
            time.sleep(0.1)  # API 호출 간 딜레이
        except ValueError:
            # 영상갯수 헤더가 없으면 무시
            pass
        ...
        try:
            ...
            self._retry_on_rate_limit(worksheet.update, last_check_cell, [[current_time]], value_input_option='USER_ENTERED')
            time.sleep(0.1)  # API 호출 간 딜레이
        except ValueError:
            # 마지막 체크일 헤더가 없으면 무시
            pass
    ...
    def find_video_row(self, sheet_name: str, video_id: str) -> Optional[int]:
        ...
        try:
            ...
            # A열 기준으로 영상 ID 찾기 - Rate Limit 대비
            video_ids = self._retry_on_rate_limit(worksheet.col_values, video_id_col_index)
    ...
    def get_all_video_ids(self, sheet_name: str) -> Dict[str, int]:
        ...
        # 영상 ID 컬럼 전체 가져오기 (한 번의 API 호출) - Rate Limit 대비
        video_ids = self._retry_on_rate_limit(worksheet.col_values, video_id_col_index)
        time.sleep(0.1)  # API 호출 간 딜레이
    ...
    def bulk_update_video_rows(self, sheet_name: str, updates: List[tuple]):
        ...
        for i in range(0, len(batch_updates), 100):
            ...
            self._retry_on_rate_limit(worksheet.batch_update, batch, value_input_option='USER_ENTERED')
            time.sleep(0.1)  # API 호출 간 딜레이
    ...
    def get_all_channel_ids(self, sheet_name: str = '채널 리스트') -> Dict[str, int]:
        """
        채널 리스트 시트의 모든 채널 ID를 한 번에 가져와서 딕셔너리로 반환

        Args:
            sheet_name: 시트 이름 (기본: '채널 리스트')
        ...
        # 채널 ID 컬럼 찾기
        channel_id_col_index = None
        for idx, header in enumerate(headers):
            normalized = normalize_header(header)
            if normalized == '채널ID':
                channel_id_col_index = idx + 1
                break

        if not channel_id_col_index:
            return {}

        # 채널 ID 컬럼 전체 가져오기 (한 번의 API 호출)
        channel_ids = self._retry_on_rate_limit(worksheet.col_values, channel_id_col_index)

        # 딕셔너리 생성 {채널 ID: 행 번호}
        channel_id_map = {}
        for row_idx, cid in enumerate(channel_ids[1:], start=2):  # 2행부터
            if cid:
                channel_id_map[cid] = row_idx

        return channel_id_map

    def extract_channel_data_from_video(self, video_data: Dict[str, any]) -> Dict[str, any]:
        """
        영상 데이터에서 채널 데이터 추출 및 변환

        Args:
            video_data: 영상 데이터 딕셔너리

        Returns:
            채널 데이터 딕셔너리
        """
        channel_data = {
            '채널 ID': video_data.get('채널 ID', ''),
            '채널링크': video_data.get('채널링크', ''),
            '채널명': video_data.get('채널명', ''),
            '구독자수': video_data.get('구독자수', 0),
            '채널전체 영상갯수': video_data.get('영상갯수', 0),
            '채널전체 조회수': video_data.get('채널 전체 조회수', 0),
            '영상당 평균 조회수': video_data.get('영상당 평균 조회수', 0),
            '개설일': video_data.get('채널 개설일', ''),
            '채널국가': video_data.get('채널국가', ''),
            '채널 디스크립션': video_data.get('채널 디스크립션', ''),
            '채널 핸들': video_data.get('채널 핸들', ''),
            '수집날짜': video_data.get('수집날짜', ''),
        }

        return channel_data

    def update_or_add_channels_to_list(self, video_data_list: List[Dict[str, any]],
                                       sheet_name: str = '채널 리스트'):
        """
        영상 데이터 리스트에서 고유 채널들을 추출하여 채널 리스트에 추가/업데이트

        Args:
            video_data_list: 영상 데이터 리스트
            sheet_name: 채널 리스트 시트 이름 (기본: '채널 리스트')
        ...
        for video_data in video_data_list:
            ...
            if channel_id and channel_id not in unique_channels:
                unique_channels[channel_id] = self.extract_channel_data_from_video(video_data)

        if not unique_channels:
            return 0, 0

        # 기존 채널 ID 목록 가져오기
        existing_channel_ids = self.get_all_channel_ids(sheet_name)

        # 업데이트할 채널과 새로 추가할 채널 분리
        channels_to_update = []
        channels_to_add = []
        ...
        for channel_id, channel_data in unique_channels.items():
            if channel_id in existing_channel_ids:
                # 기존 채널 - 업데이트 목록에 추가
                existing_row = existing_channel_ids[channel_id]
                channels_to_update.append((existing_row, channel_data))
            else:
                # 신규 채널 - 추가 목록에 추가
                channels_to_add.append(channel_data)
        ...
        if channels_to_update:
            # 전역함수가 적용되어 업데이트하지 않을 헤더 목록
            excluded_headers = CHANNEL_LIST_FORMULA_COLUMNS
            ...
            for row, data in channels_to_update:
                # 필터링된 데이터 생성 (excluded_headers 제외)
                filtered_data = {}
                for key, value in data.items():
                    normalized_key = normalize_header(key)
        ...
        # 신규 채널 추가
        if channels_to_add:
            self.bulk_append_data(sheet_name, channels_to_add, start_row=10)

        return len(channels_to_add), len(channels_to_update)
...
class YouTubeSearchGUI:
    ...
    def _setup_ui(self):
        ...
        # === 마스터 시트 섹션 ===
        master_section = ttk.LabelFrame(left_frame, text="마스터 시트", padding="10")
        master_section.grid(row=left_row, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(0, 10))
        left_row += 1

        # 스프레드시트 선택
        ttk.Label(master_section, text="스프레드시트:").grid(row=0, column=0, sticky=tk.W, pady=5, padx=(0, 5))

        # 스프레드시트 목록 정의
        self.spreadsheet_options = {
            '채널수집_2.국내롱폼': 'https://docs.google.com/spreadsheets/d/15VuwgxcfjWUGzlQIstSNcK5SnTQZtaEi3XRfyffPxEE/edit?gid=2125222511#gid=2125222511',
        ...
        self.spreadsheet_var = tk.StringVar(value='채널수집_2.국내롱폼')
        self.spreadsheet_combo = ttk.Combobox(
            master_section,
            textvariable=self.spreadsheet_var,
            values=list(self.spreadsheet_options.keys()),
            width=40,
            state='readonly'
        )
        self.spreadsheet_combo.grid(row=0, column=1, sticky=tk.W, pady=5, padx=(0, 5))
        self.spreadsheet_combo.bind('<<ComboboxSelected>>', self._on_spreadsheet_changed)
        ...
        self.duration_var = tk.StringVar(value='3분 이상')
    ...
    def _on_spreadsheet_changed(self, event=None):
        """스프레드시트 선택 변경 이벤트 - 자동 적용"""
        self._apply_spreadsheet_change()

    def _apply_spreadsheet_change(self):
        """스프레드시트 변경 적용"""
        try:
            selected_name = self.spreadsheet_var.get()
            new_url = self.spreadsheet_options.get(selected_name)

            if not new_url:
                messagebox.showerror("오류", "선택한 스프레드시트를 찾을 수 없습니다.")
                return

            # 현재 URL과 같으면 변경하지 않음
            if new_url == self.sheets_manager.spreadsheet_url:
                self._log(f"이미 '{selected_name}' 스프레드시트를 사용 중입니다.")
                return

            # 스프레드시트 변경
            self._log(f"스프레드시트 변경 중: {selected_name}")
            self.sheets_manager.spreadsheet_url = new_url
            self.sheets_manager._authenticate()

            # 시트 목록 새로고침
            self._refresh_sheet_list()
            self._refresh_playlist_list()
            self._refresh_playlist_sheet_list()
            self._refresh_channel_filter()

            self._log(f"스프레드시트 변경 완료: {selected_name}")

        except Exception as e:
            self._log(f"스프레드시트 변경 실패: {str(e)}")
            messagebox.showerror("오류", f"스프레드시트 변경 중 오류가 발생했습니다.\n{str(e)}")
    ...
    def _search_and_add_to_sheet(self):
        ...
        try:
            ...
            elif duration == 'above_3min':
                # 3분 이상 영상만 필터링 (3분 01초 이상)
                original_count = len(results)
                results = [r for r in results if r.get('영상길이(초)', 0) >= 181]
                filtered_count = original_count - len(results)
                if filtered_count > 0:
                    self._log(f"3분 이하 영상 {filtered_count}개 제외, {len(results)}개 남음")
    ...
    def _fetch_playlist_videos(self):
        ...
        try:
            ...
            # 1단계: 재생목록에서 영상 ID 목록만 먼저 가져오기 (경량)
            self._log(f"1단계: 재생목록 영상 ID 목록 조회 중...")
            try:
                all_video_ids, stats_ids = self.youtube_api.get_playlist_video_ids(playlist_id, max_results)
            except Exception as api_error:
                # YouTube API 쿼터 초과 또는 기타 오류 처리
                error_str = str(api_error)
                if 'quota' in error_str.lower() or 'quotaExceeded' in error_str:
            ...
            self._log(f"재생목록 영상 ID {len(all_video_ids)}개 조회 완료")
            ...
            # 2단계: 시트의 기존 영상 ID와 비교하여 중복 체크
            self._log(f"2단계: 기존 시트 '{target_sheet}'의 영상 ID 조회 중...")
            existing_video_ids = self.sheets_manager.get_all_video_ids(target_sheet)
            self._log(f"기존 영상 {len(existing_video_ids)}개 확인 완료")

            # 신규 영상 ID만 필터링
            new_video_ids = [vid for vid in all_video_ids if vid not in existing_video_ids]
            skipped_count = len(all_video_ids) - len(new_video_ids)

            if skipped_count > 0:
                self._log(f"{skipped_count}개 중복 영상 스킵 (기존 시트에 이미 존재)")

            if not new_video_ids:
                self._log(f"신규 영상이 없습니다. 모든 영상이 이미 시트에 존재합니다.")
                # 재생목록ID 시트의 영상갯수와 마지막 체크일 업데이트
                self.sheets_manager.update_playlist_info('재생목록ID', playlist_row, len(all_video_ids))

                # 완료 메시지
                self._log("=" * 50)
                self._log(f"[완료] 재생목록 영상 가져오기 완료")
                self._log(f"- 전체 영상 갯수: {len(all_video_ids)}개")
                self._log(f"- 신규 추가: 0개")
                self._log(f"- 중복 스킵: {skipped_count}개")
                self._log(f"- 재생목록ID 시트 업데이트 완료")
                self._log("=" * 50)
                messagebox.showinfo("완료", f"신규 영상이 없습니다.\n전체 {len(all_video_ids)}개 중 {skipped_count}개가 이미 존재합니다.")
                return

            # 3단계: 신규 영상만 20개씩 나눠서 상세 정보 가져오기
            self._log(f"3단계: 신규 영상 {len(new_video_ids)}개의 상세 정보 가져오기 중...")
            self._log(f"과부하 방지를 위해 20개씩 나눠서 처리합니다...")

            all_results = []
            total_api_calls = stats_ids['api_calls']
            total_quota_cost = stats_ids['quota_cost']

            batch_size = 20
            for i in range(0, len(new_video_ids), batch_size):
                batch_ids = new_video_ids[i:i+batch_size]
                self._log(f"  {i+1}~{min(i+batch_size, len(new_video_ids))}/{len(new_video_ids)} 영상 정보 가져오는 중...")

                try:
                    batch_results, batch_stats = self.youtube_api.get_videos_details_batch(batch_ids, playlist_id)
                    all_results.extend(batch_results)
                    total_api_calls += batch_stats['api_calls']
                    total_quota_cost += batch_stats['quota_cost']
                except Exception as api_error:
                    error_str = str(api_error)
                    if 'quota' in error_str.lower() or 'quotaExceeded' in error_str:
                        self._log("=" * 50)
            ...
            # API 통계 로그
            quota_percent = round((total_quota_cost / 10000) * 100, 2)
            self._log(f"API 호출: {total_api_calls}회")
            self._log(f"쿼터 소비: {total_quota_cost} 유닛 (일일 할당량 대비 {quota_percent}%)")
            self._log(f"신규 영상 정보 가져오기 완료: {len(all_results)}개")
            ...
            # 4단계: 구글 시트에 신규 영상 추가 (20개 단위)
            self._log(f"4단계: 구글 시트 '{target_sheet}'에 {len(all_results)}개 신규 영상 추가 중...")
            last_row_before = self.sheets_manager.get_last_row(target_sheet)

            # 20개씩 끊어서 추가 (과부하 방지)
            add_batch_size = 20
            for i in range(0, len(all_results), add_batch_size):
                batch = all_results[i:i+add_batch_size]
                self.sheets_manager.bulk_append_data(target_sheet, batch)
                self._log(f"  - {min(i+add_batch_size, len(all_results))}/{len(all_results)} 영상 추가 완료...")

            new_count = len(all_results)
            last_added_row = last_row_before + new_count
            last_added_title = all_results[-1].get('제목', '') if all_results else ''
            self._log(f"추가 완료")
            ...
            # 채널 리스트 업데이트/추가
            self._log(f"채널 리스트 업데이트 중...")
            try:
                channels_added, channels_updated = self.sheets_manager.update_or_add_channels_to_list(all_results, '채널 리스트')
                if channels_added > 0 or channels_updated > 0:
                    self._log(f"- 채널 리스트: 신규 {channels_added}개, 업데이트 {channels_updated}개")
                else:
                    self._log(f"- 채널 리스트: 변경 없음")
            except Exception as channel_error:
                self._log(f"- 채널 리스트 업데이트 실패: {str(channel_error)}")
                # 채널 리스트 업데이트 실패는 무시하고 계속 진행
            ...
            # 결과 메시지 생성
            result_msg = f"전체 영상: {len(all_video_ids)}개\n"
    ...
    def _process_single_channel(self, channel_info: Dict, show_result: bool = True):
        ...
        try:
            ...
            # 구글 시트에 추가 (중복 확인 - 신규만 추가)
            self._log(f"구글 시트 '{target_sheet}'에 신규 영상 추가 중...")
            ...
            for video_data in results:
                ...
                if video_id in existing_video_ids:
                    ...
                    skipped_videos.append((existing_row, video_title))
                    skipped_count += 1
                else:
                    # 신규 영상 - 추가 목록에 추가
            ...
            if skipped_videos:
                self._log(f"{skipped_count}개 중복 영상 스킵 (기존 시트에 이미 존재)")
                # 처음 3개 스킵된 영상 예시 표시
                sample_count = min(3, len(skipped_videos))
                for i in range(sample_count):
                    row, title = skipped_videos[i]
                    self._log(f"  예시 {i+1}: 행 {row} | '{title}'")
            ...
            if videos_to_add:
                ...
                # 20개씩 끊어서 추가 (과부하 방지)
                batch_size = 20
                for i in range(0, len(videos_to_add), batch_size):
                    batch = videos_to_add[i:i+batch_size]
                    batch_data = [video_data for video_data, _ in batch]
                    self.sheets_manager.bulk_append_data(target_sheet, batch_data, start_row=10)
                    self._log(f"  - {min(i+batch_size, len(videos_to_add))}/{len(videos_to_add)} 영상 추가 완료...")
                                ...
                                if video_count_in_sheet == 0:
                                    ...
                                    try:
                                        ...
                                        if results:
                                            ...
                                            channel_list_headers = channel_list_self._retry_on_rate_limit(worksheet.row_values, 1)
                                            channel_list_all_values = channel_list_self._retry_on_rate_limit(worksheet.get_all_values)
