...
class YouTubeSearchAPI:
    ...
    def _extract_video_data(self, video: Dict, keyword: str, channel_info: Dict = None) -> Dict:
        ...
        return {
            ...
            '영상길이(초)': duration_seconds,  # 필터링용 초 단위 길이
            '좋아요 수': like_count,
            '댓글수': comment_count,
    ...
    def get_playlist_video_ids(self, playlist_id: str, max_results: Optional[int] = None) -> tuple:
        """
        재생목록의 영상 ID 목록만 가져오기 (경량 버전)
        ...
        return all_video_ids, stats

    def get_videos_details_batch(self, video_ids: list, playlist_id: str = None) -> tuple:
        """
        영상 ID 목록으로 상세 정보를 배치로 가져오기

        Args:
            video_ids: 영상 ID 리스트
            playlist_id: 재생목록 ID (선택사항, 로깅용)
        ...
        for i in range(0, len(video_ids), 50):
            ...
            for video in videos_response.get('items', []):
                ...
                search_keyword = f'재생목록:{playlist_id}' if playlist_id else '직접조회'
                video_data = self._extract_video_data(video, search_keyword)
                results.append(video_data)
...
if __name__ == "__main__":
    ...
    def main():
        ...
        SPREADSHEET_URL = 'https://docs.google.com/spreadsheets/d/15VuwgxcfjWUGzlQIstSNcK5SnTQZtaEi3XRfyffPxEE/edit?usp=sharing'
        SHEET_NAME = '키워드 검색결과'
